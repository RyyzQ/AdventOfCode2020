...
# correctpasses part 1
correctPasses = 0

for i in range(0,len(passwords)):
    minneeded = passwords[i][0]
    maxneeded = passwords[i][1]
    character = passwords[i][2]
    password  = passwords[i][3]

    if password.count(character) >= int(minneeded) and password.count(character) <= int(maxneeded):
        correctPasses = correctPasses+1


print("Part 1:", correctPasses)

# correctpasses part 2

correctpassespart2 = 0
for i in range(0,len(passwords)):
    mustContainInPos = passwords[i][0]
    mustNotContainInPos = passwords[i][1]
    character = passwords[i][2]
    password  = passwords[i][3]

    # https://stackoverflow.com/questions/432842/how-do-you-get-the-logical-xor-of-two-variables-in-python
    if (password[int(mustContainInPos) - 1] == character) != (password[int(mustNotContainInPos) - 1] == character):
            correctpassespart2 = correctpassespart2 + 1


    # if (password[int(mustContainInPos) - 1] == character) ^ (password[int(mustNotContainInPos) - 1] == character):
    #     correctpassespart2 += 1    

print("Part 2:",correctpassespart2)